...
public class ParallaxFragment extends Fragment implements LayoutInflaterFactory {
    ...
    private int[] mParallaxAttrs = new int[]{R.attr.translationXIn,
            R.attr.translationXOut, R.attr.translationYIn, R.attr.translationYOut,R.attr.translationZIn, R.attr.translationZOut};
    ...
    private void analysisAttrs(View view, Context context, AttributeSet attrs) {
        ...
        if (array != null && array.getIndexCount() != 0) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                switch (attr) {
                    case 0:
                        tag.setTranslationXIn(array.getFloat(attr,0f));
                        break;
                    case 1:
                        tag.setTranslationXOut(array.getFloat(attr,0f));
                        break;
                    case 2:
                        tag.setTranslationYIn(array.getFloat(attr,0f));
                        break;
                    case 3:
                        tag.setTranslationYOut(array.getFloat(attr,0f));
                        break;
                    case 4:
                        tag.setTranslationZIn(array.getFloat(attr,0f));
                        break;
                    case 5:
                        tag.setTranslationZOut(array.getFloat(attr,0f));
                        break;
                }
            }
